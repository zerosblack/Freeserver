...
void initChangeTables(void)
{
...
#ifdef RENEWAL
	set_sc( CG_SPECIALSINGER	, SC_LONGING		, EFST_ENSEMBLEFATIGUE	, SCB_SPEED|SCB_ASPD );
#else
	...
	set_sc_with_vfx( SJ_FLASHKICK	, SC_FLASHKICK	, EFST_FLASHKICK		, SCB_NONE );
	set_sc( SJ_BOOKOFDIMENSION	, SC_DIMENSION	, EFST_DIMENSION		, SCB_NONE );
	set_sc( SJ_BOOKOFCREATINGSTAR	, SC_CREATINGSTAR	, EFST_CREATINGSTAR		, SCB_SPEED|SCB_HIT|SCB_FLEE );
	...
	set_sc_with_vfx( SJ_GRAVITYCONTROL	, SC_GRAVITYCONTROL	, EFST_GRAVITYCONTROL		, SCB_NONE );
	...
	set_sc( SP_SOULCURSE  , SC_SOULCURSE  , EFST_SOULCURSE  , SCB_SPEED);
	...
	StatusIconChangeTable[SC_DIMENSION1] = EFST_DIMENSION1;
	StatusIconChangeTable[SC_DIMENSION2] = EFST_DIMENSION2;
	...
	StatusDisplayType[SC_FLASHKICK]		  = BL_PC;
...
int status_base_amotion_pc(struct map_session_data* sd, struct status_data* status)
{
	...
	if ((skill_lv = pc_checkskill(sd,GS_SINGLEACTION)) > 0 && (sd->status.weapon >= W_REVOLVER && sd->status.weapon <= W_GRENADE))
		val += ((skill_lv + 1) / 2);
	if ((skill_lv = pc_checkskill(sd,SG_DEVIL)) > 0 && ((sd->class_&MAPID_THIRDMASK) == MAPID_STAR_EMPEROR || sd->status.job_level >= 50))
		val += skill_lv * 3;	
#ifdef RENEWAL
...
static unsigned short status_calc_speed(struct block_list *bl, struct status_change *sc, int speed)
{
	...
	if( sd && sd->ud.skilltimer != INVALID_TIMER && (pc_checkskill(sd,SA_FREECAST) > 0 || sd->ud.skill_id == LG_EXEEDBREAK) ) {
		...
		if( sd && sc->data[SC_HIDING] && pc_checkskill(sd,RG_TUNNELDRIVE) > 0 )
			...
			if( sc->data[SC_CURSE] )
				val = max( val, 300 );
			if (sc->data[SC_SOULCURSE]) {
				if (status_bl_has_mode(bl,MD_STATUS_IMMUNE) || bl->type == BL_PC)	
					val = max( val, 50 );
				else val = max( val, 300 );
			}
...
static short status_calc_fix_aspd(struct block_list *bl, struct status_change *sc, int aspd)
{
	...
	if (sc->data[SC_STARSTANCE])
		aspd -= sc->data[SC_STARSTANCE]->val1 * 10;
	if( sc->data[SC_SOULSHADOW] )
...
static short status_calc_aspd_rate(struct block_list *bl, struct status_change *sc, int aspd_rate)
{
	...
	if (sc->data[SC_WIND_INSIGNIA] && sc->data[SC_WIND_INSIGNIA]->val1 == 2)
		aspd_rate -= 100;
	if (sc->data[SC_STARSTANCE])
	    aspd_rate += sc->data[SC_STARSTANCE]->val2 * 10;
...
//		if (status_get_race2(bl) == RC2_GVG && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	break;
	case SC_NEWMOON:
		if (sc->data[SC_BITE]) //Star Emperor
	...
	case SC_BLESSING:
		...
		if ((!undead_flag && status->race != RC_DEMON) || bl->type == BL_PC) {
			...
			if (sc->data[SC_CURSE] || sc->data[SC_SOULCURSE]) {
...
int status_change_end_(struct block_list* bl, enum sc_type type, int tid, const char* file, int line)
{
	...
	switch(type) {
		...
		case SC_UNIVERSESTANCE:
			...
			break;
		case SC_GRAVITYCONTROL:
			status_fix_damage(bl,bl,sce->val2,clif_damage(bl,bl,gettick(),0,0,sce->val2,0,DMG_NORMAL,0,false));
